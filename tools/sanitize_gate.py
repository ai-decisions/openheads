...
# No file in this repository is exempt from any pattern. A tree that ships
# data files needs exemptions for values that legitimately appear in them;
# this one ships none, so every pattern applies everywhere.
...
# .safetensors, .bin and the pickle family passing as clean.
RESTRICTED_FILENAME = re.compile(
    r"(opensanctions|spellbook|labels_cache|probe).*\.(json|csv|parquet|ndjson)$"
